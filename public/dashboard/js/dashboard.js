...
let addressPoisoningTimeSeriesChart = null;
  ...
  loadDashboardStats()
    .then(() => {
      ...
      return Promise.all([
        ...
        loadTimeSeriesData('addressPoisoning', currentTimeframe)
      ]);
    })
    .then(() => {
      // Hide loading status
...
function setupEventListeners() {
  // Token analysis form
  document.getElementById('tokenAnalysisForm').addEventListener('submit', event => {
    event.preventDefault();
    collectTokenData();
  });

  // Transaction analysis form
  document.getElementById('transactionAnalysisForm').addEventListener('submit', event => {
    event.preventDefault();
    collectTransactionData();
  });

  // Wallet analysis form
  document.getElementById('walletAnalysisForm').addEventListener('submit', event => {
    event.preventDefault();
    collectWalletData();
  });

  // Address poisoning analysis form
  document.getElementById('addressPoisoningForm').addEventListener('submit', event => {
    event.preventDefault();
...
function updateDashboardStats(stats) {
  ...
  if (stats.addressPoisoning) {
    ...
    document.getElementById('poisoningWalletsAnalyzed').textContent = poisoningStats.totalWalletsAnalyzed;
    document.getElementById('poisoningAttemptsCount').textContent = poisoningStats.totalPoisoningAttempts;
    document.getElementById('poisoningAverageRiskScore').textContent = poisoningStats.averageRiskScore.toFixed(2);
...
function updateTimeSeriesChart(dataType, chartData) {
  ...
  switch (dataType) {
    ...
    case 'addressPoisoning':
      chart = addressPoisoningTimeSeriesChart;
      chartId = 'addressPoisoningTimeSeriesChart';
      break;
    default:
  ...
  switch (dataType) {
    ...
    case 'wallets':
      ...
      break;
    case 'addressPoisoning':
      addressPoisoningTimeSeriesChart = newChart;
      break;
  }
}
...
/**
 * Collect address poisoning data
 */
async function collectAddressPoisoningData() {
  const walletAddress = document.getElementById('poisoningWalletAddress').value.trim();
  const numTransactions = parseInt(document.getElementById('poisoningNumTransactions').value);
  ...
  showStatus(`Analyzing wallet ${walletAddress.substring(0, 8)}... for address poisoning`, true);

  try {
    const response = await fetch(`${API_BASE_URL}/dashboard/collect/address-poisoning`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    if (data.success) {
      showStatus(`Address poisoning analysis complete for ${walletAddress.substring(0, 8)}...`, false);
      ...
      showStatus(`Error analyzing address poisoning: ${data.error}`, false);
    }
  } catch (error) {
    console.error('Error collecting address poisoning data:', error);
